        ...
        <div class="sheet-body">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>Discount ({{ rtrim(rtrim(number_format((float) $disc['percent'], 2, '.', ''), '0'), '.') }}%)</td>
            ...
            <div class="totals">
                ...
                <div class="row"><span>{{ $taxLine['code'] }} ({{ rtrim(rtrim(number_format($taxLine['percent'], 2, '.', ''), '0'), '.') }}%)</span><span>₹{{ number_format((float) $taxLine['amount'], 2) }}</span></div>
                @endforeach
                @php $credit = is_array($invoice->meta) ? ($invoice->meta['credit'] ?? []) : []; @endphp
                @if(!empty($credit['applied']) && (float) ($credit['amount'] ?? 0) > 0)
                <div class="row"><span>Unused days credit{{ !empty($credit['from_plan_name']) ? ' ('.$credit['from_plan_name'].', '.(int) ($credit['remaining_days'] ?? 0).' days)' : '' }}</span><span>−₹{{ number_format((float) $credit['amount'], 2) }}</span></div>
                @endif
