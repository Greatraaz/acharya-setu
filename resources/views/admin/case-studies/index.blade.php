...
<div class="space-y-4">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <p class="text-sm text-gray-500">Manage mentorship case studies for the Vedrix Insights library.</p>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <div class="flex flex-wrap gap-2" id="case-studies-export-buttons"></div>
            <form method="GET" class="flex flex-wrap items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search title or industry…"
                ...
                @if(request()->filled('search') || request()->filled('status'))
                    <a href="{{ route('admin.case-studies.index') }}" class="text-sm text-gray-500 hover:text-gray-700 px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 transition">Reset</a>
                @endif
            </form>
        </div>
        ...
        <div class="py-16 text-center text-gray-400">
            ...
            <p class="font-medium text-gray-600">No case studies yet</p>
            <p class="text-sm mt-1">Click Add New to create your first case study.</p>
        </div>
        @else
        <div class="overflow-x-auto">
            <table id="case-studies-table" class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left px-5 py-3 font-semibold text-gray-600">Industry</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/70">
                        <td class="px-5 py-4 text-gray-500">{{ $caseStudies->firstItem() + $loop->index }}</td>
                        <td class="px-5 py-4">
                            @if($study->imageUrl())
                                <img src="{{ $study->imageUrl() }}" class="w-14 h-14 rounded-lg object-cover" alt="">
                            @else
                        ...
                        <td class="px-5 py-4 font-semibold text-gray-900 max-w-xs">
                            <div class="line-clamp-2">{{ $study->title }}</div>
                        </td>
                        <td class="px-5 py-4 text-gray-600">{{ $study->industry }}</td>
                        <td class="px-5 py-4 text-gray-600 whitespace-nowrap">{{ optional($study->created_at)->format('Y-m-d H:i:s') ?: '—' }}</td>
                        <td class="px-5 py-4">
                            @if($study->isActive())
...
<style>
    #case-studies-export-buttons .dt-button {
    ...
    #case-studies-export-buttons .dt-button:hover { background: #eff6ff !important; color: #1d4ed8 !important; border-color: #bfdbfe !important; }
    #case-studies-table_wrapper .dataTables_filter,
    #case-studies-table_wrapper .dataTables_length,
    #case-studies-table_wrapper .dataTables_info,
    #case-studies-table_wrapper .dataTables_paginate { display: none !important; }
</style>
<script>
$(function () {
    if (!$('#case-studies-table').length) return;

    var table = $('#case-studies-table').DataTable({
        paging: false,
        searching: false,
        info: false,
        ordering: true,
    ...
    table.buttons().container().appendTo('#case-studies-export-buttons');
});
</script>
@endpush
