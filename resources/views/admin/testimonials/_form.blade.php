...
<form method="POST" action="{{ $formAction }}" enctype="multipart/form-data" class="space-y-6"
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
                ...
                @if($isEdit && $testimonial->imageUrl())
                    <img src="{{ $testimonial->imageUrl() }}" alt="" class="mt-2 h-16 w-16 rounded-full object-cover border border-gray-100">
                @endif
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Testimonial Message *</label>
            <textarea name="message" id="testimonial-message" rows="10"
                      class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm">{{ old('message', $testimonial->message ?? '') }}</textarea>
    ...
    <div class="flex items-center justify-end gap-3">
        <a href="{{ route('admin.testimonials.index') }}" class="px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-gray-200 hover:bg-gray-50">Cancel</a>
