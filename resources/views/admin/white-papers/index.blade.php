...
<div class="space-y-4">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <p class="text-sm text-gray-500">Manage white papers for the Vedrix Insights library.</p>
        <a href="{{ route('admin.white-papers.create') }}"
    ...
    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
            <div class="flex flex-wrap gap-2" id="white-papers-export-buttons"></div>
            <form method="GET" class="flex flex-wrap items-center gap-2">
                ...
                @if(request()->filled('search') || request()->filled('status'))
                    <a href="{{ route('admin.white-papers.index') }}" class="text-sm text-gray-500 hover:text-gray-700 px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 transition">Reset</a>
                @endif
            </form>
        </div>
        ...
        <div class="py-16 text-center text-gray-400">
            ...
            <p class="font-medium text-gray-600">No white papers yet</p>
            <p class="text-sm mt-1">Click Add New to create your first white paper.</p>
        </div>
        @else
        <div class="overflow-x-auto">
            <table id="white-papers-table" class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/70">
                        ...
                        <td class="px-5 py-4 font-semibold text-gray-900 max-w-xs">
                            <div class="line-clamp-2">{{ $paper->title }}</div>
                        </td>
                        <td class="px-5 py-4 text-gray-600 max-w-md">
                            <div class="line-clamp-3 text-xs leading-relaxed">{{ $paper->excerpt(40) }}</div>
                        </td>
                        <td class="px-5 py-4 text-gray-600 whitespace-nowrap">{{ optional($paper->created_at)->format('Y-m-d H:i:s') ?: '—' }}</td>
                        <td class="px-5 py-4">
                            @if($paper->isActive())
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.white-papers.edit', $paper) }}"
                                   class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-green-600 text-white hover:bg-green-700" title="Edit">✎</a>
                                <form method="POST" action="{{ route('admin.white-papers.destroy', $paper) }}"
                                      onsubmit="return confirm('Delete this white paper?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
...
<style>
    #white-papers-export-buttons .dt-button {
    ...
    #white-papers-export-buttons .dt-button:hover { background: #eff6ff !important; color: #1d4ed8 !important; border-color: #bfdbfe !important; }
    #white-papers-table_wrapper .dataTables_filter,
    #white-papers-table_wrapper .dataTables_length,
    #white-papers-table_wrapper .dataTables_info,
    #white-papers-table_wrapper .dataTables_paginate { display: none !important; }
</style>
<script>
$(function () {
    if (!$('#white-papers-table').length) return;

    var table = $('#white-papers-table').DataTable({
        paging: false,
        searching: false,
        info: false,
        ordering: true,
    ...
    table.buttons().container().appendTo('#white-papers-export-buttons');
});
</script>
@endpush
